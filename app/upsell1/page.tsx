...
export default function Upsell1Page() {
  ...
  const handleAccept = async () => {
    ...
    try {
      const response = await fetch("/api/stripe/create-subscription", {
        ...
        body: JSON.stringify({
          customerId: customer,
          priceId: "price_1SUX2cCNWzvB3Neg3Qj7xiJB",
        }),
      })
      ...
      router.push(`/upsell2?email=${encodeURIComponent(email || "")}&customer=${encodeURIComponent(customer)}&upsell1_accepted=true`)
    } catch (error) {
  ...
  const handleDecline = () => {
    router.push(`/upsell2?email=${encodeURIComponent(email || "")}&customer=${encodeURIComponent(customer || "")}`)
  }
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      <div className="bg-[#002F5D] text-white text-center py-2 px-4">
        <p className="text-sm">
          Do not close this page. Important: Do not click &apos;Back&apos; or close this page.
        </p>
      </div>
